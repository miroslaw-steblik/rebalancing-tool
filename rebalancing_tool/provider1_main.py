...
provider_weekly = config.av_weekly
provider_reference = pd.read_csv(config.av_reference)
provider_static_funds_targets = pd.read_csv(config.av_static_funds_targets)
provider_output = config.av_output
# Teams webhook
...
def process():
    ...
    df = pl.load_and_preprocess_data(provider_weekly, provider_reference, pl.av_read_columns, percent=percent)
    #pl.check_columns
